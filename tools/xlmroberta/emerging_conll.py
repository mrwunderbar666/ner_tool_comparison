...
results_path = Path.cwd() / 'results' / f'roberta_{corpus}_conll.csv'
model_dir = Path.cwd() / 'tools' / 'xlmroberta' / 'tmp' / corpus

# Load and prepare data

df_train = pd.read_feather(p_corpus / 'wnut17train.feather')
df_test = pd.read_feather(p_corpus / 'emerging.dev.feather')
df_validation = pd.read_feather(p_corpus / 'emerging.test.annotated.feather')
